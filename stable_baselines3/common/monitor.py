...
class Monitor(gym.Wrapper[ObsType, ActType, ObsType, ActType]):
    ...
    def step(self, action: ActType) -> Tuple[ObsType, SupportsFloat, bool, bool, Dict[str, Any]]:
        ...
        If multi-agent, reward and info will be lists. Handles multiple agents separately.
        """
        if self.needs_reset:
        ...
        # Check if we are dealing with multi-agent environment (reward and info as list)
        if isinstance(reward, list) and isinstance(infos, list):
            # Multi-agent case: process each agent separately
            for i in range(len(reward)):
                self.rewards.append(float(reward[i]))  # Record each agent's reward
                ...
                if terminated[i] or truncated[i]:
                    ...
                    for key in self.info_keywords:
                        ep_info[key] = infos[i].get(key, None)  # Use the i-th agent's info
                    ...
                    infos[i]["episode"] = ep_info  # Store episode info for the i-th agent

        else:
            # Single-agent case: keep original logic
            ...
            if terminated or truncated:
                ...
                ep_info = {"r": round(ep_rew, 6), "l": ep_len, "t": round(time.time() - self.t_start, 6)}
                for key in self.info_keywords:
                    ep_info[key] = infos.get(key, None)
                self.episode_returns.append(ep_rew)
                self.episode_lengths.append(ep_len)
                self.episode_times.append(time.time() - self.t_start)
                ep_info.update(self.current_reset_info)
