...
class TD3(OffPolicyAlgorithm):
    ...
    def reset_lstm(self):
        # 重置 LSTM 隐状态和细胞状态
        self.hidden = th.zeros(3, self.batch_size, self.actor.actor_features_extractor.lstm.hidden_size).to(self.device)
        self.cell = th.zeros(3, self.batch_size, self.actor.actor_features_extractor.lstm.hidden_size).to(self.device)
    ...
    def _sample_action(
        ...
        """
        Sample an action for both agents according to the exploration policy.
        This is either done by sampling the policy's distribution,
        ...
        :param n_envs: Number of environments (should be 2 for two agents).
        :return: action for both agents to take in the environment
    ...
    def _store_transition(
            ...
            buffer_action: Tuple[np.ndarray, np.ndarray],
            new_obs: Tuple[Union[np.ndarray, Dict[str, np.ndarray]], Union[np.ndarray, Dict[str, np.ndarray]]],
            reward: Tuple[np.ndarray, np.ndarray],
            dones: Tuple[np.ndarray, np.ndarray],
            infos: Tuple[Dict[str, Any], Dict[str, Any]],
    ) -> None:
